...
## Посадка построек на землю (/proto/)

`assets/topdown/grounding.json` → `python tools/topdown_grounding.py` →
`proto/sprites_grounded/` (+ `index.json`). Для построек из списка берётся
исходный спрайт: объект (маска вырезанной версии, без «гребёнки») возвращается
целиком, от нарисованного цоколя остаётся тонкий контакт вокруг основания
(`keep`, затем `fade` — доли высоты), остальное убрано; меряется реальная линия
контакта. /proto/ берёт такой спрайт вместо вырезанного, а тень рисует узкой
полосой по линии контакта, а не эллипсом у кромки холста. Холст не меняется,
поэтому расстановка и следы те же. `--check` — в CI.
