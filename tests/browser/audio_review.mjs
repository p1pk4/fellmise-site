 ...
 *
 * "On" plays the production files (assets/audio/**, the format the browser
 * picks), so requests and bytes are the real ones. Nothing in the repository
 * is written.
 */
...
import { AUDIO_CONFIG, isAudioUrl } from './lib/audio_fixture.mjs';
...
for (const on of [false, true]) {
  ...
  rows.push({ title: on ? 'sound ON (production files)' : 'muted (default)', shots });
...
Config: \`assets/topdown/audio.json\` — ${AUDIO_CONFIG.biomes.length} ambients and ${AUDIO_CONFIG.transitions.length} transition SFX, status: ${[...new Set([...AUDIO_CONFIG.biomes, ...AUDIO_CONFIG.transitions].map((e) => e.status))].join(', ')}; sources WebM/Opus, fallback M4A/AAC (one format per session).
...
| | muted (default) | sound on (production files) |
|---|---|---|
...
| audio bytes, all | ${m.muted.bytes} | ${m.on.bytes} |
