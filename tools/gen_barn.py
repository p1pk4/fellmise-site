...
TASKS = [
    # Third pass. The first template left it isometric; the second was not run
    # on it because the pick looked acceptable at the time, and in the scene it
    # is not — the roof still dominates.
    ("barn", "wooden barn with large front doors and a hay loft window"),
]
...
def build(t, obj, seed):
    ...
    wf["12"]["inputs"]["filename_prefix"] = cc.unique_prefix("front3")
    return wf
