 ...
 *
 * dwell — минимальное время показа бита, мс: при энергичной прокрутке окно
 * range пролетается за доли секунды, и текст не успевали прочитать. Текст
 * может задержаться после range, но не дольше dwell и не дальше hardExit.
 * hardExit — 45% раскрытия следующей сцены (band0 + (gate0 + .45·(gate1−gate0))·len
 * по параметрам journey.js): дальше старый текст лежал бы поверх почти целиком
 * открывшейся новой сцены. У дома принудительного ухода нет.
