"""Build the retired root placeholder ("заглушка") of fellmise.com.

    python tools/build_stub.py                 # -> out/stub/ (archive; touches nothing live)
    python tools/build_stub.py --write-root    # overwrite the production / and /ru/ (deliberate)

NOT the source of the root any more: / and /ru/ are the journey, generated by
tools/build_pages.py. This tool is kept to rebuild the placeholder for reference,
and it writes to out/ unless it is told otherwise in as many words.
...
def main(argv=()):
    """The placeholder is retired: / and /ru/ are the journey (tools/build_pages.py).

    Writing it out therefore goes to out/stub/ and nowhere near the production
    documents. Overwriting them needs --write-root, typed on purpose."""
    root = "--write-root" in argv
    out = ROOT if root else ROOT / "out" / "stub"
    if root:
        print("build_stub: overwriting the PRODUCTION root with the old placeholder")
    (out / "ru").mkdir(parents=True, exist_ok=True)
    (out / "stub.css").write_text(CSS, encoding="utf-8")
    (out / "index.html").write_text(build("en"), encoding="utf-8")
    (out / "ru" / "index.html").write_text(build("ru"), encoding="utf-8")
    for p in ("index.html", "ru/index.html", "stub.css"):
        print(f"{p:<16} {(out / p).stat().st_size / 1024:.1f} KB  ->  {(out / p).relative_to(ROOT)}")
