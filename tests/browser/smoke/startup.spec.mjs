 ...
 * запуска ниже 1280, прятало живую сцену стилями при data-mode=live.
 *
...
test('A: / and /ru/ pick the same mode at the same viewport', async ({ browser, baseURL }) => {
  const rows = [];
  for (const width of [1920, 1440, 1366, 1280, 1279, 1152, 1024, 600]) {
    const modes = [];
    for (const url of ['/', '/ru/']) {
    ...
    expect(modes[0], `mode at ${width}`).toBe(modes[1]);
    expect(modes[0], `mode at ${width}`).toBe(width >= 1280 ? 'live' : 'static');
  }
...
test('B: CSS follows the chosen mode at the breakpoint and after resize', async ({ browser, baseURL }) => {
  ...
  // живой старт на 1920, затем окно уже 1280: страница не должна опустеть
  ...
  await expect.poll(() => page.evaluate(shown), { timeout: 5_000 }).toMatchObject({ meaningful: true, mode: 'static', sound: false });
  await page.context().close();
});
